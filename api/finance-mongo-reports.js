...
export function computeWalletLeft(entries, vouchers, bankAccount) {
    ...
    const cashIn = (vouchers || []).filter((d) => d.kind === 'IN' && isCashNotes(d.notes) && bookStatus(d) !== 'void');
    ...
    for (const t of funding) {
        const amt = Math.abs(Number(t.amount) || 0);
        const linked = linkedByTxn.get(String(t.id)) || 0;
        carry = round2(carry + amt - linked);
    }

    // Classic: bucketLeft = finalCarry when funding exists, else opening (or 0).
    const finalCarry = funding.length ? carry : (openingAmt != null ? openingAmt : 0);

    const receiptsAmt = round2(cashIn.reduce((s, d) => s + Math.abs(Number(d.amount) || 0), 0));
    const depositedAmt = round2(cashIn
        .filter((d) => bookStatus(d) === 'linked' && d.transaction_id)
        .reduce((s, d) => s + Math.abs(Number(d.amount) || 0), 0));
    const receiptsOnHand = round2(receiptsAmt - depositedAmt);

    const deskDeposits = round2((entries || [])
        .filter((t) => t.type === 'IN' && reportWallet(t) === 'BANK')
        .reduce((s, t) => s + Math.max(0, parseFloat(t.cash_desk_deposit) || 0), 0));

    return round2(finalCarry + receiptsOnHand - deskDeposits);
}
