...
import { fetchRolePermissionKeys, saveSocietyRolePageAccess } from './pageAccess.js';
import { persistMongoPolicy } from '@new/rbacMongoClient.js';
...
/** Map legacy permission keys (accounts.view / accounts.edit) onto CRUD. */
...
/** Nav still uses page/module maps — fill them from CRUD so Roles only edits one grid. */
export function derivePagesAndModulesFromCrud(crudByRole, roles = MATRIX_ROLE_KEYS) {
    const pages = {};
    const moduleEnabled = {};
    const catalog = buildPageCatalog();
    roles.forEach((roleKey) => {
        const roleCrud = crudByRole?.[roleKey] || {};
        pages[roleKey] = {};
        catalog.forEach((page) => {
            pages[roleKey][page.route] = pageAllowedByCrud(page, roleCrud);
        });
        ...
        MODULE_CATALOG.forEach(({ key }) => {
            ...
            moduleEnabled[roleKey][key] = defaultModuleEnabled(key, roleKey, [], pages[roleKey]);
        });
    });
...
/** Persist role policy. New UI → one Mongo document; classic → Postgres rows. */
