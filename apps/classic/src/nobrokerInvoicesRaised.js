...
export async function applyNoBrokerInvoicesRaisedImport(parsed) {
  ...
  for (const month of months) {
    ...
    if (delErr) {
      if (/nobroker_invoices_raised/i.test(delErr.message) && /does not exist|schema cache/i.test(delErr.message)) {
        throw new Error(
          'Table nobroker_invoices_raised is missing. Run docs/scripts/sql/supabase_nobroker_invoices_raised.sql in Supabase SQL Editor, then retry.',
        );
      }
      throw new Error(`Could not clear existing month ${month}: ${delErr.message}`);
    }
  }
  ...
  for (let i = 0; i < payload.length; i += chunkSize) {
    ...
    let { data, error } = await supabase
      ...
      .insert(chunk)
      .select('*');
    if (error && /raw/i.test(error.message) && /column|schema cache/i.test(error.message)) {
      // Table exists but raw column not migrated yet — still import mapped fields.
      const withoutRaw = chunk.map(({ raw, ...rest }) => rest);
      ...
      if (!error) {
        console.warn(
          '[nobroker] Imported without raw column. Run alter in docs/scripts/sql/supabase_nobroker_invoices_raised.sql',
        );
      }
    }
    if (error) {
      if (/nobroker_invoices_raised/i.test(error.message) && /does not exist|schema cache/i.test(error.message)) {
        throw new Error(
          'Table nobroker_invoices_raised is missing. Run docs/scripts/sql/supabase_nobroker_invoices_raised.sql in Supabase SQL Editor, then retry.',
        );
      }
      if (/raw/i.test(error.message)) {
        throw new Error(
          'Column raw is missing on nobroker_invoices_raised. Run the ALTER in docs/scripts/sql/supabase_nobroker_invoices_raised.sql, then re-upload.',
        );
      }
  ...
  // Refresh finance domain; seed local state if RPC path still omits this table.
  await pullState({ domain: 'finance' });
  if (!portalState.finances) portalState.finances = {};
  if (!portalState.finances.nobrokerInvoicesRaised?.length && inserted.length) {
    portalState.finances.nobrokerInvoicesRaised = inserted;
  } else if (inserted.length) {
    // Prefer DB rows for the months we just wrote.
    const monthSet = new Set(months.map((m) => String(m).slice(0, 10)));
    const keep = (portalState.finances.nobrokerInvoicesRaised || [])
      .filter((r) => !monthSet.has(String(r.billing_month || '').slice(0, 10)));
    portalState.finances.nobrokerInvoicesRaised = [...inserted, ...keep];
  }
...
export const getNoBrokerInvoicesRaised = () =>
  portalState.finances?.nobrokerInvoicesRaised || [];
