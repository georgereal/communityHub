/** Finance-New clone (duesAging.js) */
...
import { mongoInsert } from './mongoWrite.js';
import {
    clearResidentsCache,
    getBillToForInvoice,
    getResidents,
...
export const getLastReminder = (invoiceId) => {
    const logs = (fnFinances().maintenanceReminderLog || [])
...
export const getAgingSummary = (asOf = todayIso()) => {
    ...
    (fnFinances().maintenanceInvoices || [])
        .filter((inv) => invoiceBalance(inv) > 0.001)
        .filter((inv) => invoiceMatchesBlock(inv))
        .forEach((inv) => {
            const bucket = getAgingBucket(inv, asOf);
...
export async function logReminderSend({
    ...
    let sent_by = null;
    try {
        const { data: { user } } = await (await import('@auth/authClient.js')).authClient.auth.getUser();
        sent_by = user?.id || null;
    } catch { /* ignore */ }
    await mongoInsert('maintenance_reminder_log', {
...
export const sendReminderForInvoice = async (invoiceId, { force = false } = {}) => {
    ...
    const { pullState } = await import('./pull.js');
    await pullState({ packs: ['billing'] });
    return result;
};
...
export const renderAgingPage = async () => {
    ...
    set('fn-aging-sum-current', formatMoney(summary.current));
    set('fn-aging-sum-30', formatMoney(summary['1-30']));
    set('fn-aging-sum-60', formatMoney(summary['31-60']));
    set('fn-aging-sum-90', formatMoney(summary['61-90']));
    set('fn-aging-sum-90plus', formatMoney(summary['90+']));

    const blockNote = document.getElementById('fn-aging-block-note');
    if (blockNote) {
    ...
    const list = document.getElementById('fn-aging-reminder-list');
    if (!list) return;
...
export const initDuesAgingUi = () => {
    document.getElementById('fn-aging-export-btn')?.addEventListener('click', () => {
        exportAgingExcel().catch((err) => alert(err?.message || 'Export failed.'));
    });
    document.getElementById('fn-aging-bulk-send-btn')?.addEventListener('click', async () => {
    ...
    document.getElementById('fn-aging-select-all')?.addEventListener('click', () => {
