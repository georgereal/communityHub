/** Finance-New clone of classic module (finances.js) — Mongo-backed, isolated DOM (fn-*). */
...
function formatAllocationSummary(txnId) {
    const allocs = (fnFinances().maintenanceAllocations || []).filter((a) => a.transaction_id === txnId);
    if (!allocs.length) return '';
    return allocs.map((a) => {
        const inv = (fnFinances().maintenanceInvoices || []).find((i) => i.id === a.invoice_id);
        const flat = inv?.flat_label || inv?.unit_label || inv?.period_label || 'Flat';
        const period = inv?.period_label || 'Invoice';
        return `${flat} · ${period} (₹${parseFloat(a.amount).toLocaleString('en-IN')})`;
    }).join('; ');
}

function syncMaintenanceIncomeSection(...args) {
    void loadMaintenanceBilling().then((m) => m.syncMaintenanceIncomeSection(...args));
}
...
const findVendorRow = (name) => {
    ...
    return (fnFinances().vendors || []).find((row) => row.name?.trim().toLowerCase() === needle);
};

const findSubCategoryRow = (category, name) => {
    const needle = String(name || '').trim().toLowerCase();
    return (fnFinances().subCategories || []).find(
...
async function rememberSubCategory(apartment_id, category, name) {
    ...
    try {
        await mongoUpsert('expense_sub_categories', payload, { rehydrate: false });
        const idx = (fnFinances().subCategories || []).findIndex((v) => String(v.id) === String(payload.id));
        if (idx >= 0) fnFinances().subCategories[idx] = payload;
        else fnFinances().subCategories = [...(fnFinances().subCategories || []), payload];
    } catch (err) {
        console.warn('Could not cache sub-category:', err.message);
    }
}
...
export const processFinances = () => {
    ...
    fnFinances().txns.forEach((t) => {
        ...
        if (t.type === 'OUT' && d.toDateString() === now.toDateString()) outToday += amt;
    });

    // Petty cash KPI = Wallet Left (Bills float), not CASH-wallet ledger sum.
    let cash = 0;
    try {
        cash = Number(getCashWalletLeft()) || 0;
    } catch {
        fnFinances().txns.forEach((t) => {
            const amt = parseFloat(t.amount);
            const wallet = t.wallet || 'CASH';
            if (wallet !== 'CASH') return;
            if (t.type === 'IN') cash += amt;
            else cash -= amt;
        });
    }
    ...
    const k = (id) => document.getElementById(id) || document.getElementById(`fn-${id}`);
    if (k('cash-balance')) {
        k('cash-balance').textContent = fmt(cash);
        k('cash-balance').title = 'Wallet Left from Bills & receipts (Petty Cash float)';
        ...
        if (k('cash-txn-count')) {
            const n = fnFinances().txns.length;
...
async function uploadReceiptFile(apartmentId, txnId, file, index, subfolder = '') {
    ...
    if (!authClient) throw new Error('Sign in required to upload receipts.');
    const { error } = await authClient.storage.from(RECEIPT_BUCKET).upload(base, file, {
        upsert: true,
...
async function deleteReceiptPaths(paths) {
    ...
    await authClient.storage.from(RECEIPT_BUCKET).remove(paths);
}
...
async function resolveReceiptUrl(pathOrUrl) {
    ...
    const { data, error } = await authClient.storage.from(RECEIPT_BUCKET).createSignedUrl(storagePath, 3600);
    if (error) throw error;
...
bindFinanceNewWindow('viewAttachmentGallery', async (entries, startIndex = 0) => {
...
export const saveCashData = async () => {
    ...
    try {
        ...
        const allocations = isIncome && cat === 'Maintenance Collection'
            ...
            : [];
        const result = await postFnMutation('saveTransaction', {
            apartment_id,
            transaction: payload,
            allocations,
...
bindFinanceNewWindow('saveCashData', saveCashData);

bindFinanceNewWindow('previewReceiptFile', async (file) => {
    if (!file) return;
    await window.viewAttachmentGallery([file], 0);
});

bindFinanceNewWindow('viewReceipt', async (pathOrUrl, index = 0) => {
    if (!pathOrUrl) return;
    await window.viewAttachmentGallery([pathOrUrl], index);
});

bindFinanceNewWindow('viewReceipts', async (txnId, index = 0) => {
...
bindFinanceNewWindow('closeReceiptViewer', () => {
    cleanupReceiptViewer();
...
const applyCashModalChrome = ({ isIncome, isBill }) => {
    ...
    const walletSection = document.querySelector('#fn-expense-form-view .expense-form__section--wallet');
    const bankSection = document.getElementById('expense-bank-section');
    const incomeWallet = document.querySelector('#fn-income-form-view .expense-form__section--wallet');
...
bindFinanceNewWindow('openExpense', (wallet = 'CASH', { asBill = true } = {}) => {
    const modal = document.getElementById('cash-modal');
    if (!modal) return;
    portalState.editingTxnId = null;
    portalState.editingFinanceDocId = null;
...
bindFinanceNewWindow('openIncome', (wallet = 'CASH', { asBill = true } = {}) => {
    const modal = document.getElementById('cash-modal');
    if (!modal) return;
    portalState.editingTxnId = null;
    portalState.editingFinanceDocId = null;
...
bindFinanceNewWindow('openBillExpense', () => window.openExpense('CASH', { asBill: true }));
bindFinanceNewWindow('openBillIncome', () => window.openIncome('CASH', { asBill: true }));

/** Edit / view an existing finance document in the same Add bill / Add receipt modal. */
bindFinanceNewWindow('openFinanceDocumentEdit', (doc, { readOnly = false } = {}) => {
    if (!doc) return;
    const modal = document.getElementById('cash-modal');
    if (!modal) return;
...
bindFinanceNewWindow('openFinanceDocumentView', (doc) => window.openFinanceDocumentEdit(doc, { readOnly: true }));
...
bindFinanceNewWindow('openBankSnapshot', () => {
...
bindFinanceNewWindow('openCash', (direction = 'OUT', wallet = 'CASH') => {
...
bindFinanceNewWindow('openLedgerExpense', () => openLedgerLineModal('expense'));
bindFinanceNewWindow('openLedgerIncome', () => openLedgerLineModal('income'));
...
export const saveLedgerLineData = async () => {
    ...
    const existing = editingId
        ? (fnFinances().txns || []).find((t) => t.id === editingId)
        : null;
...
bindFinanceNewWindow('saveLedgerLineData', saveLedgerLineData);
...
export const syncAccountsHeaderActions = (sv) => {
    ...
    // Bank Sync is ledger/recon only — hide on bills, reports, etc.
    const canUseBankSync = can('accounts.bank_sync');
    ...
    applyFinancePageHeader({
        titleId: 'fn-accounts-page-title',
        descId: 'fn-accounts-page-desc',
        help: standalone
            ? ACCOUNTS_PAGE_HELP.activity
            : help,
    });
    // Close help when switching tabs so content isn't stale under the wrong title.
    const helpPop = document.getElementById('fn-accounts-page-help');
    const helpBtn = document.getElementById('fn-accounts-page-help-btn');
    if (helpPop && !helpPop.hidden) {
        helpPop.hidden = true;
        helpBtn?.setAttribute('aria-expanded', 'false');
    }
};
bindFinanceNewWindow('syncAccountsHeaderActions', syncAccountsHeaderActions);
...
export const renderAccountsSubviewTabs = (activeSubview = 'ledger') => {
    const nav = document.querySelector('#view-finance-new-accounts .accounts-subview-tabs');
    if (!nav) return;
    const pages = getFinanceNewAccountsPages();
...
const wireAccountsHeaderActionButtons = () => {
    const root = document.getElementById('fn-accounts-header-actions');
    if (!root || root.dataset.wired === '1') return;
    root.dataset.wired = '1';
    document.getElementById('fn-btn-ledger-add-expense')?.addEventListener('click', () => window.openLedgerExpense());
    document.getElementById('fn-btn-ledger-add-income')?.addEventListener('click', () => window.openLedgerIncome());
    document.getElementById('fn-btn-bill-add-expense')?.addEventListener('click', () => window.openBillExpense());
    document.getElementById('fn-btn-bill-add-income')?.addEventListener('click', () => window.openBillIncome());
...
export const delTxn = async (id) => {
    ...
    if (confirm('Delete Record?')) {
        await postFnMutation('deleteTransaction', {
...
bindFinanceNewWindow('delTxn', delTxn);

bindFinanceNewWindow('editTxn', (id) => {
    const t = fnFinances().txns.find((x) => x.id == id);
    if (!t) return;
    // Ledger edit uses the lean passbook modal — not the bill/receipt form.
    openLedgerLineModal(t.type === 'IN' ? 'income' : 'expense', t);
});

const FN_ACCOUNTS_SUBVIEW_ROUTES = {
    reports: 'fn-reports',
    ledger: 'fn-ledger',
...
const ROUTE_TO_ACCOUNTS_SUBVIEW = Object.fromEntries(
    Object.entries(FN_ACCOUNTS_SUBVIEW_ROUTES).map(([subview, route]) => [route, subview]),
);

export const syncAccountsSubViewTabs = (route) => {
    const subview = ROUTE_TO_ACCOUNTS_SUBVIEW[route] || 'ledger';
    const root = document.getElementById('view-finance-new-accounts');
    const nav = root?.querySelector('.accounts-subview-tabs');
    if (nav && !nav.querySelector('[data-accounts-subview]')) {
        renderAccountsSubviewTabs(subview);
        wireAccountsSubviewTabClicks();
    }
    root?.querySelectorAll('[data-accounts-subview]').forEach((btn) => {
...
const wireAccountsSubviewTabClicks = () => {
    document.querySelectorAll('#view-finance-new-accounts [data-accounts-subview]').forEach((btn) => {
        if (btn.dataset.wired) return;
        btn.dataset.wired = '1';
        btn.addEventListener('click', () => {
            const route = FN_ACCOUNTS_SUBVIEW_ROUTES[btn.dataset.accountsSubview];
...
export const initAccountsSubViewTabs = () => {
    ...
    if (!isMpa) {
        ...
        wireAccountsSubviewTabClicks();
        wireFinancePageHelp({
            btnId: 'fn-accounts-page-help-btn',
            popoverId: 'fn-accounts-page-help',
            getHelp: () => {
                const active = document.querySelector('#view-finance-new-accounts [data-accounts-subview].active')?.dataset?.accountsSubview;
                const tabsHidden = document.querySelector('#view-finance-new-accounts .accounts-subview-tabs')?.hidden;
    ...
    if (!document.documentElement.dataset.fnCrudAccessLedgerWired) {
        document.documentElement.dataset.fnCrudAccessLedgerWired = '1';
        document.addEventListener('crud-access-loaded', () => {
            const isMpa = document.documentElement.dataset.financeApp === '1';
            if (!isMpa && !document.getElementById('view-finance-new-accounts')?.classList.contains('active')) return;
            void (async () => {
                try {
                ...
                try {
                    ...
                    if (document.getElementById('fn-subview-finance-docs')) renderFinanceDocumentsPage();
                } catch { /* ignore */ }
                try {
                    const { renderExpensePlanPage } = await import('./expensePlan.js');
                    if (document.getElementById('fn-subview-expense-plan')) renderExpensePlanPage();
                } catch { /* ignore */ }
...
const initLedgerSearch = () => {
    const input = document.getElementById('fn-cash-search');
    if (!input || input.dataset.wired) return;
    input.dataset.wired = '1';
    let debounce = null;
    const onSearchInput = () => {
...
const showLedgerRecalcStatus = (message = 'Recalculating calculated balances… please wait.') => {
    const el = document.getElementById('fn-ledger-recalc-status');
    if (!el) return;
    el.hidden = false;
...
const syncLedgerOpeningFields = () => {
    ...
    const dateEl = document.getElementById('fn-ledger-opening-date');
    const amountEl = document.getElementById('fn-ledger-opening-amount');
    const summary = document.getElementById('fn-ledger-opening-summary');
...
const openLedgerOpeningPicker = ({ focusAmount = true } = {}) => {
    const picker = document.getElementById('fn-ledger-opening-picker');
    if (!picker) return;
    syncLedgerOpeningFields();
    picker.open = true;
    if (focusAmount) {
        requestAnimationFrame(() => document.getElementById('fn-ledger-opening-amount')?.focus());
    }
};

const initLedgerOpeningBalance = () => {
    const saveBtn = document.getElementById('fn-ledger-opening-save');
    if (!saveBtn || saveBtn.dataset.wired) return;
    saveBtn.dataset.wired = '1';

    document.getElementById('fn-ledger-bank-hint')?.addEventListener('click', (e) => {
        if (!e.target.closest('#ledger-bank-hint-open')) return;
        openLedgerOpeningPicker();
    });

    saveBtn.addEventListener('click', async () => {
        const date = document.getElementById('fn-ledger-opening-date')?.value;
        const raw = document.getElementById('fn-ledger-opening-amount')?.value?.trim();
        ...
        try {
            await withButtonBusy(saveBtn, 'Saving…', async () => {
                ...
                try {
                    const { saveBankOpeningBalance } = await import('./bankReconciliation.js');
                    await saveBankOpeningBalance(date, amount, { pull: true });
                    document.getElementById('fn-ledger-opening-picker')?.removeAttribute('open');
                    processFinances();
                    renderCashLedger();
                } finally {
    ...
    btn.addEventListener('click', async () => {
        ...
        try {
            await withButtonBusy(btn, 'Recalculating…', async () => {
                const meta = getLedgerBalanceMeta();
                showLedgerRecalcStatus(
                    meta.dirtyFromDate
                        ? `Recalculating ledger balances from ${meta.dirtyFromDate}…`
                        : 'Recalculating ledger balances…',
                );
                try {
                    await recalculateLedgerBalances({ full: !meta.dirtyFromDate });
                    const { recalculateBankStatementBalances } = await import('./bankReconciliation.js');
...
const initLedgerTableControls = () => {
    ...
    const host = document.getElementById('fn-cash-ledger-items');
...
bindFinanceNewWindow('switchSubView', async (sv) => {
    // Old cash-float tab folded into Bills & receipts
    if (sv === 'cash-float') sv = 'finance-docs';
    const views = {
        ledger: 'fn-subview-ledger',
        'finance-docs': 'fn-subview-finance-docs',
        'expense-plan': 'fn-subview-expense-plan',
        reports: 'fn-subview-reports',
        'invoices-raised': 'fn-subview-invoices-raised',
        'bank-recon': 'fn-subview-bank-recon',
        activity: 'fn-subview-activity',
    };
    ...
    try {
        const { loadAccountsSubview } = await import('./load.js');
        await loadAccountsSubview(sv);
    } catch (err) {
        console.error('[fn] subview load:', err);
    ...
    if (sv === 'finance-docs') {
        const m = await import('./financeDocuments.js');
        m.initFinanceDocumentsPage();
        m.renderFinanceDocumentsPage();
    }
    if (sv === 'expense-plan') {
        const m = await import('./expensePlan.js');
        m.initExpensePlanPage();
        m.renderExpensePlanPage();
    }
    if (sv === 'invoices-raised') {
        const m = await import('./invoicesRaisedPage.js');
        m.initInvoicesRaisedPage();
        m.renderInvoicesRaisedPage();
    }
    if (sv === 'bank-recon') {
        const m = await import('./bankReconciliation.js');
        m.initBankReconciliationUi?.();
        m.renderBankReconciliation?.();
    }
    if (sv === 'activity') {
        const list = document.getElementById('fn-activity-log-list');
        if (list) {
            list.innerHTML = '<p class="maintenance-dues-empty">Activity log stays on classic Administration (Postgres).</p>';
        }
    }
    if (sv === 'ledger') {
        processFinances();
