/**
 * App UI mode: `new` (default, Mongo property/finance) vs `classic` (Supabase screens).
 * Auth, RBAC, and society membership always stay on Supabase.
 */
...
export function paintUiModeButtons() {
    ...
    document.querySelectorAll('[data-ui-mode-toggle]').forEach((btn) => {
        ...
        btn.title = classic
            ? 'Using classic (Supabase). Click to switch to New (Mongo).'
            : 'Using New. Click to switch to classic (Supabase). Ops, security, and portal stay on classic data.';
