...
export const getBillingGroupUnits = () => portalState.finances.maintenanceBillingGroupUnits || [];

export const getBillingGroups = () =>
    (portalState.finances.maintenanceBillingGroups || [])
...
export async function saveBillingGroup(payload) {
    ...
    const { error } = await supabase.from('maintenance_billing_groups').upsert(row);
    if (error) throw new Error(error.message);

    const { error: delErr } = await supabase
        .from('maintenance_billing_group_units')
        .delete()
        .eq('group_id', groupId);
    if (delErr && !/maintenance_billing_group_units/i.test(delErr.message)) {
    ...
    const { error: insErr } = await supabase.from('maintenance_billing_group_units').insert(memberRows);
...
export async function deleteBillingGroup(id) {
    ...
    const { error } = await supabase.from('maintenance_billing_groups').delete().eq('id', id);
...
const renderSelectedUnitsList = () => {
    ...
    document.getElementById('billing-group-clear-selected')?.addEventListener('click', () => {
...
const renderSuggestPanel = async () => {
    ...
    document.getElementById('billing-group-suggest-create')?.addEventListener('click', async () => {
...
window.openBillingGroupsModal = openBillingGroupsModal;
window.closeBillingGroupsModal = closeBillingGroupsModal;
