/** Finance-New clone of classic module (invoicesRaisedPage.js) — Mongo-backed, isolated DOM (fn-*). */
...
const renderColumnPicker = (chargeHeads, rawKeys) => {
    const el = document.getElementById('fn-raised-col-picker');
    if (!el) return;
...
const syncClearFiltersBtn = () => {
    const btn = document.getElementById('fn-raised-clear-filters');
...
export function renderInvoicesRaisedPage() {
    ...
    const meta = document.getElementById('fn-raised-page-meta');
    if (meta) {
    ...
    const tbody = document.getElementById('fn-raised-table-body');
    const thead = document.getElementById('fn-raised-table-head');
    if (thead) {
...
async function handleUpload(file) {
    if (!file) return;
    const btn = document.getElementById('fn-raised-page-upload-btn');
    try {
...
export function initInvoicesRaisedPage() {
    ...
    document.getElementById('fn-raised-page-upload-btn')?.addEventListener('click', () => {
        document.getElementById('fn-raised-page-file')?.click();
    });
    document.getElementById('fn-raised-page-file')?.addEventListener('change', (e) => {
        const file = e.target.files?.[0];
        if (file) void handleUpload(file);
        e.target.value = '';
    });
    document.getElementById('fn-raised-clear-filters')?.addEventListener('click', () => {
        clearFilters();
    });
    document.getElementById('fn-raised-col-picker')?.addEventListener('change', (e) => {
    ...
    const tableWrap = document.querySelector('#fn-subview-invoices-raised .raised-table-wrap')
        || document.querySelector('#subview-invoices-raised .raised-table-wrap');
