/** Finance-New clone (cashFloat.js) */
...
export const computeCashFloatStore = (txns = fnFinances()?.txns || []) => {
...
export function renderCashFloatPage() {
  const root = document.getElementById('fn-cash-float-root');
  if (!root) return;
...
export function initCashFloatPage() {
  const uploadBtn = document.getElementById('fn-cash-float-upload-btn');
  const fileInput = document.getElementById('fn-cash-float-file');
  const templateBtn = document.getElementById('fn-cash-float-template-btn');
  const addBtn = document.getElementById('fn-cash-float-add-expense');
