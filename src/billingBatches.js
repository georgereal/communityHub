...
const getInvoiceDisplayLabel = (inv) => {
    if (inv.billing_group_id) {
        const group = portalState.finances.maintenanceBillingGroups?.find((g) => g.id === inv.billing_group_id);
        const memberIds = (portalState.finances.maintenanceBillingGroupUnits || [])
...
export const getBatches = () =>
    [...(portalState.finances.maintenanceBillingBatches || [])]
        .sort((a, b) => (b.created_at || '').localeCompare(a.created_at || ''));

export const getBatchInvoices = (batchId) =>
    (portalState.finances.maintenanceInvoices || []).filter((inv) => inv.batch_id === batchId);

export const getBatchSkips = (batchId) =>
    (portalState.finances.maintenanceBillingBatchSkips || []).filter((s) => s.batch_id === batchId);
...
export const renderBillingRunsList = () => {
    const list = document.getElementById('billing-runs-list');
    if (!list) return;
