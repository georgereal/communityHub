-- Notice delivery channels — run in Supabase SQL Editor
-- (copy of docs/scripts/sql/supabase_notice_delivery.sql)
...
create policy "user_notifications read own" on public.user_notifications for select using (user_id = auth.uid());
create policy "user_notifications insert staff" on public.user_notifications for insert with check (
  exists (select 1 from public.user_apartments ua join public.profiles p on p.id = ua.user_id
    where ua.user_id = auth.uid() and ua.apartment_id = user_notifications.apartment_id
    and coalesce(p.role, 'resident_viewer') in ('admin', 'property_manager')));

create policy "user_notifications update own" on public.user_notifications for update using (user_id = auth.uid());
...
create policy "sms_outbox read" on public.sms_outbox for select using (
  exists (select 1 from public.user_apartments ua join public.profiles p on p.id = ua.user_id
    where ua.user_id = auth.uid() and ua.apartment_id = sms_outbox.apartment_id
    and coalesce(p.role, 'resident_viewer') in ('admin', 'property_manager', 'accounts_manager')));

create policy "sms_outbox insert" on public.sms_outbox for insert with check (
  exists (select 1 from public.user_apartments ua join public.profiles p on p.id = ua.user_id
    where ua.user_id = auth.uid() and ua.apartment_id = sms_outbox.apartment_id
    and coalesce(p.role, 'resident_viewer') in ('admin', 'property_manager', 'accounts_manager')));

create policy "sms_outbox update" on public.sms_outbox for update using (
  exists (select 1 from public.user_apartments ua join public.profiles p on p.id = ua.user_id
    where ua.user_id = auth.uid() and ua.apartment_id = sms_outbox.apartment_id
    and coalesce(p.role, 'resident_viewer') in ('admin', 'property_manager', 'accounts_manager')));
